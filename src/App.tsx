...
export default function App() {
  ...
  const orbitRaising = useMemo(() => {
    ...
    return { deltaV: baseline + dragDeltaV, dragDeltaV, transferTime, dragForce, power, powerMargin: powerAvailable - power, impulse: mass * (baseline + dragDeltaV), semiMajorDecay };
  ...
  const deorbit = useMemo(() => {
    ...
    const dragAcceleration = 0.5 * (cd * area / mass) * density * Math.pow(ORBITAL_VELOCITY, 2);
    let deltaV = 0, semiMajorDecay = 0;
    if (deorbitAltitude > 100) {
      const r2 = EARTH_RADIUS + 100000;
      const aTransfer = (r1 + r2) / 2;
      const baseline = Math.abs(Math.sqrt(MU / r1) * (1 - Math.sqrt((2 * r2) / (r1 + r2))));
      semiMajorDecay = (-2 * Math.pow(aTransfer, 1.5) * dragAcceleration) / Math.sqrt(MU);
      const transferTime = Math.PI * Math.sqrt(Math.pow(aTransfer, 3) / MU);
      const deltaADrag = Math.abs(semiMajorDecay * transferTime);
      const dragDeltaV = (deltaADrag * ORBITAL_VELOCITY) / (2 * r1);
      deltaV = Math.max(0, baseline - dragDeltaV);
    }
    return { deltaV, semiMajorDecay, ballisticCoefficient: mass / (cd * area) };
      ...
      <div className="mx-auto max-w-7xl grid lg:grid-cols-[360px_1fr] gap-8">
        <aside className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 p-8 rounded-3xl h-fit space-y-6 shadow-2xl">
          <header className="mb-8">
            <h1 className="text-3xl font-black italic tracking-tighter text-white">JivaJet <span className="text-cyan-400 not-italic font-light">Mission</span></h1>
            <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500 font-bold mt-1">Multi-Service Orbital Engine</p>
          </header>
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4 border-t border-slate-800 space-y-2">
              <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-2">Active Services</p>
              {(["orbitRaising", "stationKeeping", "deorbiting"] as Service[]).map((s) => (
                <button key={s} onClick={() => toggleService(s)} className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-sm capitalize ${services.includes(s) ? 'border-cyan-500/50 bg-cyan-500/10 text-white' : 'border-slate-800 text-slate-500 hover:border-slate-700'}`}>
                  <div className={`w-2 h-2 rounded-full ${services.includes(s) ? 'bg-cyan-400 shadow-[0_0_8px_cyan]' : 'bg-slate-700'}`} />
                  {s.replace(/([A-Z])/g, ' $1')}
                </button>
              ))}
            </div>
        ...
        <main className="space-y-6">
          {orbitRaising && (
            <Section title="Orbit Raising" color="text-cyan-400" borderColor="border-cyan-500/20">
              ...
              <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
                <ResultCard title="Total ΔV" value={`${orbitRaising.deltaV.toFixed(2)}`} unit="m/s" />
                <ResultCard title="Drag Force" value={orbitRaising.dragForce.toExponential(2)} unit="N" />
                <ResultCard title="Power Req." value={orbitRaising.power.toFixed(1)} unit="W" />
                <ResultCard title="Raise Time" value={(orbitRaising.transferTime / 60).toFixed(1)} unit="min" />
              </div>
            </Section>
          )}

          {stationKeeping && (
            <Section title="Station Keeping" color="text-emerald-400" borderColor="border-emerald-500/20">
              <div className="grid md:grid-cols-3 gap-4 mb-8">
                <InputGroup label="Orbit Alt" unit="km" value={orbitAltitude} onChange={setOrbitAltitude} />
                <InputGroup label="Life" unit="Yrs" value={duration} onChange={setDuration} />
                <InputGroup label="Ion Vel." unit="m/s" value={ionVelocity} onChange={setIonVelocity} />
              </div>
              <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
                <ResultCard title="Total Impulse" value={stationKeeping.impulse.toExponential(2)} unit="N·s" />
                <ResultCard title="Propellant" value={stationKeeping.propellant.toFixed(4)} unit="kg" />
                <ResultCard title="Thrust Req." value={stationKeeping.thrust.toExponential(2)} unit="N" />
                <ResultCard title="Power Req." value={stationKeeping.power.toFixed(1)} unit="W" />
              </div>
            </Section>
          )}

          {deorbit && (
            <Section title="Deorbiting" color="text-rose-400" borderColor="border-rose-500/20">
              <div className="max-w-xs mb-8">
                <InputGroup label="Start Altitude" unit="km" value={deorbitAltitude} onChange={setDeorbitAltitude} />
              </div>
              <div className="grid sm:grid-cols-3 gap-4">
                <ResultCard title="Ballistic Coeff." value={deorbit.ballisticCoefficient.toFixed(2)} unit="kg/m²" />
                <ResultCard title="Required ΔV" value={deorbit.deltaV.toFixed(2)} unit="m/s" />
                <ResultCard title="Decay Rate" value={deorbit.semiMajorDecay.toExponential(2)} unit="m/s" />
              </div>
            </Section>
          )}
        </main>
...
function Section({ title, children, color, borderColor }: any) {
  return (
    <div className={`bg-slate-900/30 border ${borderColor} rounded-3xl p-8 backdrop-blur-md animate-in fade-in slide-in-from-bottom-4 duration-500`}>
      <h2 className={`text-xl font-black uppercase tracking-widest mb-8 ${color}`}>{title}</h2>
      {children}
    </div>
  );
}
...
function InputGroup({ label, unit, value, onChange, ...props }: any) {
  return (
    <div className="relative group">
      <label className="block text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest group-focus-within:text-cyan-400 transition-colors">{label}</label>
      ...
      <span className="absolute right-4 bottom-3.5 text-[10px] font-bold text-slate-700 uppercase">{unit}</span>
    </div>
  );
}
...
function SelectGroup({ label, value, onChange, options }: any) {
  return (
    <div>
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-lg focus:border-cyan-500/50 outline-none transition-all appearance-none">
